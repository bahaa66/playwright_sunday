...
if config_env() not in [:dev, :test] do
  ...
  config :cogynt_workstation_ingest,
    session_domain: System.get_env("COGYNT_SESSION_DOMAIN"),
    session_key: System.get_env("COGYNT_AUTH_SESSION_KEY"),
    signing_salt: signing_salt,
    enable_dev_tools: (System.get_env("ENABLE_DEV_TOOLS") || "true") == "true",
    authoring_version: System.get_env("COGYNT_AUTHORING_VERSION")
  ...
  config :kafka, :application,
    ...
    replication_factor: System.get_env("KAFKA_TOPIC_REPLICATION_FACTOR", "1") |> String.to_integer(),
  ...
  config :cogynt_workstation_ingest, :event_pipeline,
    ...
    producer_stages: System.get_env("EVENTPIPELINE_PRODUCER_STAGES", "10") |> String.to_integer()
  ...
  config :cogynt_workstation_ingest, :rpc,
    ...
    cogynt_auth_service_port: System.get_env("COGYNT_AUTH_SERVICE_PORT", "4999") |> String.to_integer()
else
  config :cogynt_workstation_ingest, CogyntWorkstationIngestWeb.Endpoint,
