defmodule Cluster.Strategy.Kubernetes.Debug do
  @moduledoc ~S"""
  ...
  defp get_nodes(%State{topology: topology, config: config, meta: meta}) do
    ...
    cond do
      app_name != nil and selector != nil ->
        ...
        IO.inspect(cluster_name, label: "CLUSTER NAME")
        IO.inspect(service_name, label: "SERVICE NAME")
        IO.inspect(selector, label: "APP SELECTOR")
        IO.inspect(ip_lookup_mode, label: "IP LOOKUP MODE")
        IO.inspect(master_name, label: "MASTER NAME")
  ...
  defp format_node(
    ...
    :"#{app_name}@#{hostname}.#{service_name}.#{namespace}.svc.#{cluster_name}.local:4000/healthz"
  end
